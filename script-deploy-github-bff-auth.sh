...
# Script automatizado de deploy para BFF Auth
# Fluxo: SNAPSHOT → RELEASE → Próxima versão
# Uso: ./script-deploy-github-bff-auth.sh [version] [up]
# Exemplos:
#   ./script-deploy-github-bff-auth.sh 1.0.0 up    # Versão + Docker
#   ./script-deploy-github-bff-auth.sh 1.0.0       # Versão + GitHub Only
#   ./script-deploy-github-bff-auth.sh up          # Auto + Docker
#   ./script-deploy-github-bff-auth.sh             # Auto + GitHub Only
...
build_image() {
    ...
    mkdir -p .bin
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o .bin/bff-auth ./cmd/bff-auth
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o .bin/healthcheck ./deploy/healthcheck
...
show_final_info() {
    ...
    if [ "$mode" = "up" ]; then
        ...
        echo "  - Health Check: http://localhost:8381/health"
...
# Função principal
main() {
    echo -e "${CYAN}🚀 Deploy Automatizado - BFF Auth${NC}"
    echo "=========================================="
